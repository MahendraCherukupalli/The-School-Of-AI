...
def log_tool_call(tool_name, success):
    if tool_name not in tool_stats:
        tool_stats[tool_name] = {"calls": 0, "success": 0, "fail": 0}
...
# Utility function for extracting data from document chunks
def extract_data_from_chunk(text, data_type="price"):
    """
    Extract structured data from document chunks
    
    Parameters:
    text (str): The text to extract data from
    data_type (str): The type of data to extract ('price', 'date', 'person', 'amount')
    
    Returns:
    str: The extracted data or a message indicating no data was found
    """
    text = text.replace('\n', ' ')
    
    if data_type == "price":
        # Look for various price formats (₹/Rs/INR + number + cr/crore/lakh)
        patterns = [
            r'(?:₹|Rs\.?|INR)\s*(\d+(?:\.\d+)?)\s*(?:cr|crore)',
            r'(\d+(?:\.\d+)?)\s*(?:cr|crore)\s*(?:₹|Rs\.?|INR)',
            r'(?:₹|Rs\.?|INR)\s*(\d+(?:\.\d+)?)\s*(?:lakh)',
            r'(\d+(?:\.\d+)?)\s*(?:lakh)\s*(?:₹|Rs\.?|INR)',
            r'(?:USD|US\$|\$)\s*(\d+(?:\.\d+)?)\s*(?:billion|million)',
            r'(?:paid|bought|purchased|acquired).*?(?:for|at|worth|costing)\s*(?:₹|Rs\.?|INR|USD|US\$|\$)?\s*(\d+(?:\.\d+)?)\s*(?:cr|crore|lakh|billion|million)',
            r'(?:price|cost|value).*?(?:of|is|was)\s*(?:₹|Rs\.?|INR|USD|US\$|\$)?\s*(\d+(?:\.\d+)?)\s*(?:cr|crore|lakh|billion|million)'
        ]
        ...
        # Look for person names - this is simplified and might need improvement
        patterns = [
            r'([A-Z][a-z]+(?:\s[A-Z][a-z]+){1,3})',
            r'Mr\.?\s([A-Z][a-z]+(?:\s[A-Z][a-z]+){1,3})',
            r'Ms\.?\s([A-Z][a-z]+(?:\s[A-Z][a-z]+){1,3})',
            r'Mrs\.?\s([A-Z][a-z]+(?:\s[A-Z][a-z]+){1,3})'
        ]
...
def make_tool_proxy(tool_name: str, mcp):
    async def _tool_fn(*args):
        try:
            ...
            log_tool_call(tool_name, success=True)
            return result
        except Exception as e:
            log_tool_call(tool_name, success=False)
            raise  # re-raise so the rest of your error handling works as before
    return _tool_fn

from action.executor import tool_stats